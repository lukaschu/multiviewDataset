...
# create subfolders
os.makedirs(os.path.join(destination_folder, 'train', 'complete'), exist_ok=True)
os.makedirs(os.path.join(destination_folder, 'train', 'partial'), exist_ok=True)
os.makedirs(os.path.join(destination_folder, 'valid', 'complete'), exist_ok=True)
os.makedirs(os.path.join(destination_folder, 'valid', 'partial'), exist_ok=True)
os.makedirs(os.path.join(destination_folder, 'test', 'complete'), exist_ok=True)
os.makedirs(os.path.join(destination_folder, 'test', 'partial'), exist_ok=True)
...
def copy_files(src_files, dest_folder):
    for file in src_files:
        shutil.copy(os.path.join(data_folder, 'complete', file), os.path.join(dest_folder, 'complete', file))
        for partial_pc in os.listdir(os.path.join(data_folder, 'partial', file.split('.')[0])):
            # create destination folder
            os.makedirs(os.path.join(dest_folder, 'partial', file.split('.')[0]), exist_ok=True)
            shutil.copy(os.path.join(data_folder, 'partial', file.split('.')[0], partial_pc), os.path.join(dest_folder, 'partial', file.split('.')[0], partial_pc))
...
json_file_path = os.path.join(data_folder, 'split_info.json')
